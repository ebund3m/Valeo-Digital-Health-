...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  // Fix: strip leading "Dr. " so we never get "Dr. Dr. Name"
    ...
    <div className="min-h-screen flex overflow-hidden" style={{ background:"#F0F4F4" }}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-6 border-b border-white/10">
          <div>
            <span className="text-white text-lg block" style={{ fontFamily:"var(--font-dm-serif)" }}>Valeo</span>
            <span className="text-xs tracking-widest uppercase" style={{ color:"#4ECDC4" }}>Doctor Portal</span>
          </div>
        ...
        <div className="mx-4 mt-5 mb-2 rounded-xl p-3 flex items-center gap-3"
          ...
          <div className="overflow-hidden">
            {/* Display "Dr. CleanName" — never doubles */}
            <p className="text-white text-sm font-medium truncate">Dr. {cleanName}</p>
            <p className="text-xs truncate" style={{ color:"rgba(255,255,255,0.4)" }}>Health Psychologist</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b"
          style={{ background:"rgba(240,244,244,0.92)", backdropFilter:"blur(12px)", borderColor:"rgba(10,46,53,0.08)" }}>
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-black/5" style={{ color:"#0A2E35" }}>
              <Menu size={20}/>
            </button>
            <h1 className="text-lg font-medium" style={{ fontFamily:"var(--font-dm-serif)", color:"#0A2E35" }}>
              {navItems.find(i => i.href === pathname)?.label ?? "Dashboard"}
            </h1>
          </div>

          <div className="flex items-center gap-3">
            <button className="relative p-2 rounded-lg hover:bg-black/5 transition-colors" style={{ color:"#4A5568" }}>
              <Bell size={18}/>
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full" style={{ background:"#E8604C" }}/>
            </button>
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium"
